...
function getIDB(): Promise<IDBDatabase> {
  ...
  dbPromise = new Promise<IDBDatabase>((resolve, reject) => {
    ...
    // 1-second timeout guard to prevent Safari / WebKit from hanging indefinitely on connection locks
    const timeoutId = setTimeout(() => {
      reject(new Error("IndexedDB open timed out on this browser"));
    }, 1000);
    ...
    try {
      ...
      request.onblocked = () => {
        console.warn("IndexedDB open blocked by another tab/version");
        clearTimeout(timeoutId);
        // Do not reject immediately on blocked, but handle gracefully
      };
